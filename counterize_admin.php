...
function counterize_options_page()
{
	...
	// For the Refresh User-agent table button
	if (isset($_POST['refreshua']))
	{
		counterize_update_all_userAgents();
		counterize_updateText(__("User-agent table refreshed!","counterize"));
	}
	...
	<div class="wrap">
		...
		<table width="100%" cellpadding="3" cellspacing="3" class="editform">
			...
			<tr class="alternate">
				<td>
					<?php
						_e("Refresh the user-agent table", "counterize");
						echo "<br><small>";
						_e("(You should do this if you just upgraded from a previous version!)", "counterize");
						echo"</small>";
					?>
				</td>
				<td>
					<form action="" method="post">
						<input type="submit" name="refreshua" value="<?php _e("Refresh", "counterize"); ?>" />
					</form>
				</td>
			</tr>
			<tr>
